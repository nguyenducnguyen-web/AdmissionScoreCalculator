...
namespace DoanC_
{
    public partial class frmPhuongthuc2_3 : Form
    {
        private DuLieu dulieu;

        public frmPhuongthuc2_3(DuLieu dulieu)
        {
            InitializeComponent();
            InitializeData();
            this.dulieu = dulieu;
        ...
        private void btnkiemtradiemthmh_Click(object sender, EventArgs e)
        {
            ...
            if (tohop == "A00 (Toán, Vật lý, Hóa học)" && T < 6 && VL < 6 && HH < 6)
            {
                MessageBox.Show("Điểm trung bình mỗi môn trong tổ hợp không đạt từ 6.0 trở lên. Vui lòng chọn phương thức xét tuyển khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (tohop == "A01 (Toán, Vật lý, Tiếng Anh)" && T < 6 && VL < 6 && TA < 6)
            {
                MessageBox.Show("Điểm trung bình mỗi môn trong tổ hợp không đạt từ 6.0 trở lên. Vui lòng chọn phương thức xét tuyển khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (tohop == "D01 (Toán, Ngữ văn, Tiếng Anh)" && T < 6 && NV < 6 && TA < 6)
            {
                MessageBox.Show("Điểm trung bình mỗi môn trong tổ hợp không đạt từ 6.0 trở lên. Vui lòng chọn phương thức xét tuyển khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);

            }
            else if (tohop == "D62 (Ngữ văn, Tiếng Anh, Khoa học tự nhiên)" && T < 6 && TA < 6 && khtn < 6)
            {
                MessageBox.Show("Điểm trung bình mỗi môn trong tổ hợp không đạt từ 6.0 trở lên. Vui lòng chọn phương thức xét tuyển khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (tohop == "D68 (Ngữ văn, Tiếng Anh, Khoa học xã hội)" && NV < 6 && TA < 6 && khxh < 6)
            {
                MessageBox.Show("Điểm trung bình mỗi môn trong tổ hợp không đạt từ 6.0 trở lên. Vui lòng chọn phương thức xét tuyển khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (tohop == "D96(Toán, Tiếng Anh, Khoa học xã hội)" && T < 6 && TA < 6 && khxh < 6)
            {
                MessageBox.Show("Điểm trung bình mỗi môn trong tổ hợp không đạt từ 6.0 trở lên. Vui lòng chọn phương thức xét tuyển khác.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else if (tohop == "")
        ...
        private void frmPhuongthuc2_3_FormClosed(object sender, FormClosedEventArgs e)
        {
            Application.Exit();
